...
const UpdateGroupChatModal = ({
  ...
  const handleSearch = async (query) => {
    ...
    if (!query) {
      ...
      toast.info("Search to add users");
    }
    setLoading(true);
    try {
      const config = {
  ...
  const handleAddMember = async (userToAdd) => {
    if (userToAdd.email === "guest@mail.com") {
          ...
          <ModalFooter>
            <Accordion allowToggle width="100%">
              <AccordionItem>
                ...
                <AccordionPanel>
                  <Text>
                    By Exiting the Group, you will not be able to access old
                    chat and all the chat media will be deleted
                  </Text>
                  <Button
                    mt="2"
                    colorScheme="red"
